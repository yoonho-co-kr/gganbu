...
function getCharacterCopyText(character: CharacterSummary) {
  const shortServerName = character.serverName.replace(/\s+/g, "").slice(0, 2) || character.serverName.slice(0, 2);
  return `${character.name}[${shortServerName}]`;
}
...
function CopyIcon({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" aria-hidden="true">
      ...
      <path d="M5 15V7a2 2 0 012-2h8" strokeWidth="1.8" strokeLinecap="round" />
    </svg>
  );
}

function CharacterCopyButton({ character, dense = false }: { character: CharacterSummary; dense?: boolean }) {
  const copyText = getCharacterCopyText(character);
  ...
  return (
    <button
      ...
      title={`${copyText} 복사`}
      className={`inline-flex shrink-0 items-center justify-center rounded-md border border-neutral-700 bg-neutral-900 text-neutral-400 transition hover:border-neutral-500 hover:bg-neutral-800 hover:text-neutral-100 ${
        dense ? "h-5 w-5" : "h-6 w-6"
      }`}
    >
      <CopyIcon className={dense ? "h-3 w-3" : "h-3.5 w-3.5"} />
    </button>
  );
}
    ...
    >
      {slotLayout ? (
        <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-x-2 gap-y-1">
          <div className={`flex min-w-0 items-center gap-1.5 font-bold text-neutral-100 ${dense ? "text-sm" : "text-md"}`}>
            ...
            <CharacterCopyButton character={character} dense={dense} />
          </div>
          <div
          ...
          <div className="flex items-center gap-2">
            <div className="min-w-0 flex-1">
              <div className="flex min-w-0 items-center gap-1.5">
                ...
                <CharacterCopyButton character={character} />
              </div>
